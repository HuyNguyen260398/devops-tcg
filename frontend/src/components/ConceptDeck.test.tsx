...
describe("ConceptDeck", () => {
  it("renders the title above its counter with enabled carousel controls", () => {
    ...
    expect(within(header!).getByLabelText("Card 1 of 21")).toHaveTextContent(
      "01 / 21",
    );
  ...
  it("follows the finger while the drag is in progress", () => {
    ...
    expect(screen.getByText("01 / 21")).toBeInTheDocument();
  });
  ...
  it("goes back on a rightward swipe", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Redis Cluster card, front shown" }),
    ).toBeInTheDocument();
  ...
  it("places each neighbouring card in its own signed slot", () => {
    ...
    expect(slotOf("redis-cluster")).toBe("-1");
  });
  ...
  it("hands every slot its depth and its side", () => {
    ...
    expect(depthAndSide("redis-cluster")).toEqual(["1", "-1"]);
    expect(depthAndSide("nginx")).toEqual(["2", "1"]);
    expect(depthAndSide("rbac")).toEqual(["2", "-1"]);
  });
  ...
  it("hides every off-centre slot from assistive technology and focus", () => {
    ...
    for (const id of ["cdn", "nginx", "redis-cluster", "rbac"]) {
  ...
  describe("shuffle control", () => {
    ...
    it("slides the deck right to left and stops on the card it dealt", async () => {
      ...
      expect(screen.getByLabelText("Card 9 of 21")).toHaveTextContent(
        "09 / 21",
      );
    });
