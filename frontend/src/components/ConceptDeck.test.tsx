...
describe("ConceptDeck", () => {
  ...
  it("goes back on a rightward swipe", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "AWS Security Group card, front shown",
      }),
    ).toBeInTheDocument();
    expect(slotOf("aws-security-group")).toBe("0");
  });
  ...
  it("places each neighbouring card in its own signed slot", () => {
    ...
    expect(slotOf("aws-security-group")).toBe("-1");
  });
  ...
  it("stages the rank beyond the spread so no card can pop in", () => {
    ...
    expect(slotOf("aws-transit-gateway")).toBe("-2");
  ...
  it("hands every slot its depth and its side", () => {
    ...
    expect(depthAndSide("aws-security-group")).toEqual(["1", "-1"]);
    expect(depthAndSide("nginx")).toEqual(["2", "1"]);
    expect(depthAndSide("aws-transit-gateway")).toEqual(["2", "-1"]);
  });
