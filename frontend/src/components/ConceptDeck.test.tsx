...
describe("ConceptDeck", () => {
  ...
  it("goes back on a rightward swipe", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "AWS Route Table card, front shown",
      }),
    ).toBeInTheDocument();
    expect(slotOf("aws-route-table")).toBe("0");
  });
  ...
  it("places each neighbouring card in its own signed slot", () => {
    ...
    expect(slotOf("aws-route-table")).toBe("-1");
  });
  ...
  it("hands every slot its depth and its side", () => {
    ...
    expect(depthAndSide("aws-route-table")).toEqual(["1", "-1"]);
    expect(depthAndSide("nginx")).toEqual(["2", "1"]);
    expect(depthAndSide("aws-cidr")).toEqual(["2", "-1"]);
  });
  ...
  it("hides every off-centre slot from assistive technology and focus", () => {
    ...
    for (const id of ["cdn", "nginx", "aws-route-table", "aws-cidr"]) {
