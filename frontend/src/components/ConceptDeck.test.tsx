...
describe("ConceptDeck", () => {
  ...
  it("goes back on a rightward swipe", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "AWS Stateless Services card, front shown",
      }),
    ).toBeInTheDocument();
    expect(slotOf("aws-stateless-services")).toBe("0");
  });
  ...
  it("places each neighbouring card in its own signed slot", () => {
    ...
    expect(slotOf("aws-stateless-services")).toBe("-1");
  });
  ...
  it("stages the rank beyond the spread so no card can pop in", () => {
    ...
    expect(slotOf("kubernetes-deployment")).toBe("-2");
  ...
  it("hands every slot its depth and its side", () => {
    ...
    expect(depthAndSide("aws-stateless-services")).toEqual(["1", "-1"]);
    expect(depthAndSide("nginx")).toEqual(["2", "1"]);
    expect(depthAndSide("kubernetes-deployment")).toEqual(["2", "-1"]);
  });
