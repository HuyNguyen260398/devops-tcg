...
describe("ConceptDeck", () => {
  ...
  it("goes back on a rightward swipe", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "Kubernetes ReplicaSet card, front shown",
      }),
    ).toBeInTheDocument();
    expect(slotOf("kubernetes-replicaset")).toBe("0");
  });
  ...
  it("places each neighbouring card in its own signed slot", () => {
    ...
    expect(slotOf("kubernetes-replicaset")).toBe("-1");
  });
  ...
  it("stages the rank beyond the spread so no card can pop in", () => {
    ...
    expect(slotOf("aws-stateful-services")).toBe("-2");
  ...
  it("hands every slot its depth and its side", () => {
    ...
    expect(depthAndSide("kubernetes-replicaset")).toEqual(["1", "-1"]);
    expect(depthAndSide("nginx")).toEqual(["2", "1"]);
    expect(depthAndSide("aws-stateful-services")).toEqual(["2", "-1"]);
  });
