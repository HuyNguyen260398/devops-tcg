...
describe("ConceptDeck", () => {
  it("renders the title above its counter with enabled carousel controls", () => {
    ...
    expect(within(header!).getByLabelText("Card 1 of 17")).toHaveTextContent(
      "01 / 17",
    );
  ...
  it("leaves Space to a focused arrow button instead of flipping", async () => {
    ...
    expect(screen.getByText("02 / 17")).toBeInTheDocument();
  });
  ...
  it("goes back on a rightward swipe", () => {
    ...
    expect(
      screen.getByRole("button", { name: "OIDC card, front shown" }),
    ).toBeInTheDocument();
  ...
  it("stages the rank beyond the spread so no card can pop in", () => {
    ...
    expect(slotOf("aws-iam-policy")).toBe("-2");
  ...
  it("hands every slot its depth and its side", () => {
    ...
    expect(depthAndSide("oidc")).toEqual(["1", "-1"]);
    expect(depthAndSide("nginx")).toEqual(["2", "1"]);
    expect(depthAndSide("aws-iam-policy")).toEqual(["2", "-1"]);
  });
  ...
  it("hides every off-centre slot from assistive technology and focus", () => {
    ...
    for (const id of ["cdn", "nginx", "oidc", "aws-iam-policy"]) {
  ...
  describe("shuffle control", () => {
    ...
    it("slides the deck right to left and stops on the card it dealt", async () => {
      ...
      expect(screen.getByLabelText("Card 9 of 17")).toHaveTextContent(
        "09 / 17",
      );
    });
