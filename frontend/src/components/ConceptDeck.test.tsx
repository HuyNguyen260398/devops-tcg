...
describe("ConceptDeck", () => {
  ...
  it("goes back on a rightward swipe", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "AWS VPC Endpoint card, front shown",
      }),
    ).toBeInTheDocument();
    expect(slotOf("aws-vpc-endpoint")).toBe("0");
  });
  ...
  it("places each neighbouring card in its own signed slot", () => {
    ...
    expect(slotOf("aws-vpc-endpoint")).toBe("-1");
  });
  ...
  it("stages the rank beyond the spread so no card can pop in", () => {
    ...
    expect(slotOf("aws-network-acl")).toBe("-2");
  ...
  it("hands every slot its depth and its side", () => {
    ...
    expect(depthAndSide("aws-vpc-endpoint")).toEqual(["1", "-1"]);
    expect(depthAndSide("nginx")).toEqual(["2", "1"]);
    expect(depthAndSide("aws-network-acl")).toEqual(["2", "-1"]);
  });
  ...
  it("hides every off-centre slot from assistive technology and focus", () => {
    ...
    for (const id of ["cdn", "nginx", "aws-vpc-endpoint", "aws-network-acl"]) {
