...
describe("ConceptDeck", () => {
  it("renders the title above its counter with enabled carousel controls", () => {
    ...
    expect(within(header!).getByLabelText("Card 1 of 16")).toHaveTextContent(
      "01 / 16",
    );
  ...
  it("goes back on a rightward swipe", () => {
    ...
    expect(
      screen.getByRole("button", { name: "AWS IAM Policy card, front shown" }),
    ).toBeInTheDocument();
  ...
  it("places each neighbouring card in its own signed slot", () => {
    ...
    expect(slotOf("aws-iam-policy")).toBe("-1");
  });
  ...
  it("stages the rank beyond the spread so no card can pop in", () => {
    ...
    expect(slotOf("aws-iam-role")).toBe("-2");
  ...
  it("hands every slot its depth and its side", () => {
    ...
    expect(depthAndSide("aws-iam-policy")).toEqual(["1", "-1"]);
    expect(depthAndSide("nginx")).toEqual(["2", "1"]);
    expect(depthAndSide("aws-iam-role")).toEqual(["2", "-1"]);
  });
  ...
  it("hides every off-centre slot from assistive technology and focus", () => {
    ...
    for (const id of ["cdn", "nginx", "aws-iam-policy", "aws-iam-role"]) {
  ...
  describe("shuffle control", () => {
    ...
    it("slides the deck right to left and stops on the card it dealt", async () => {
      ...
      expect(screen.getByLabelText("Card 9 of 16")).toHaveTextContent(
        "09 / 16",
      );
    });
    ...
    it("lands on the same card at once when motion is turned down", async () => {
      ...
      try {
        ...
        expect(track()).not.toHaveAttribute("data-spinning");
        expect(activeTitle()).toBe("Lambda Throttle");
        expect(screen.getByText("09 / 16")).toBeInTheDocument();
      } finally {
        vi.unstubAllGlobals();
      }
    });
