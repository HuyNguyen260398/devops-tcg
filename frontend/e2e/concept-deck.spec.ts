...
test("navigates the shipped deck infinitely with a live counter", async ({
  ...
  for (let position = 1; position <= images.length; position += 1) {
    await expect(
      page.getByText(`${position.toString().padStart(2, "0")} / 19`),
    ).toBeVisible();
...
test("centers the stacked header and shows faded adjacent cards", async ({
  ...
  const counter = page.getByLabel("Card 1 of 19");
...
test("navigates the deck with a touch swipe", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText("02 / 19")).toBeVisible();
...
// Eight to eleven cards fly past a shuffle, so from the top of a nineteen-card
...
const LANDINGS = ["09 / 19", "10 / 19", "11 / 19", "12 / 19"];
