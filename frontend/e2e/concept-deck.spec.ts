...
test("navigates the shipped deck infinitely with a live counter", async ({
  ...
  for (let position = 1; position <= images.length; position += 1) {
    await expect(
      page.getByText(`${position.toString().padStart(2, "0")} / 16`),
    ).toBeVisible();
...
test("centers the stacked header and shows faded adjacent cards", async ({
  ...
  const counter = page.getByLabel("Card 1 of 16");
...
test("navigates the deck with a touch swipe", async ({ page }, testInfo) => {
  ...
  await expect(page.getByText("02 / 16")).toBeVisible();
...
// Eight to eleven cards fly past a shuffle, so from the top of a sixteen-card
...
// length would land on 06 through 09, which overlaps this set at 09.
const LANDINGS = ["09 / 16", "10 / 16", "11 / 16", "12 / 16"];
